...
- **Cron for nic:** A script at `/home/nic/bin/sync-authorized-keys.sh` (mode 700, only nic can read) fetches [GitHub nicbotes.keys](https://github.com/nicbotes.keys) every 6 hours and **merges** those keys into `authorized_keys` for both nic and black_thorn (it never silently removes existing keys). black_thorn cannot read the script or nic’s home.
